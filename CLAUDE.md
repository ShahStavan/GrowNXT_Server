...
- **A GPU worker cap will be needed again.** Workers are threads sharing a process, so each puts its own layout and table models on the same card, and more than one under ~8 GB VRAM thrashes. `resolve_workers()` owned that decision and **went with `ingestion/batch.py` in the vector removal** -- the reasoning stands, the code does not. Reintroduce it wherever the qualitative pipeline gains concurrency.
