...
def _synthetic_document() -> ParsedDocument:
    ...
    mdna.blocks = [
        Block(
            kind="prose",
            text="Revenue for the year grew to 926,163 million. " * 6,
            page_start=1,
            page_end=1,
        ),
        Block(
            kind="table",
            text="Operating Activities 149,316 169,426 (20,110)",
            page_start=2,
            page_end=2,
        ),
        Block(
            kind="prose",
            text="Operating margin declined on wage revision. " * 6,
            page_start=2,
            page_end=2,
        ),
    ]
    ...
    risk.blocks = [
        Block(
            kind="prose",
            text="Client concentration remains a principal risk. " * 6,
            page_start=3,
            page_end=3,
        ),
    ]
...
def _long_document() -> ParsedDocument:
    ...
    section.blocks = [
        Block(
            kind="prose",
            text=f"Paragraph {index} discusses segment performance in detail. " * 5,
...
def check_layout(report: Report) -> None:
    ...
    report.check(
        ...
        "gutters=%s (expected one at x=100)"
        % [(round(a), round(b)) for a, b in gutters],
    )
    ...
    with_heading.append(
        [
            word("HEADING", 20.0, 90.0),
            word("SPANS", 60.0, 90.0),
            word("EVERY", 100.0, 90.0),
            word("COLUMN", 140.0, 90.0),
    ...
    for index in range(10):
        solid.append(
            [
                word("full", 20.0, 100.0 + index),
                word("width", 60.0, 100.0 + index),
                word("row", 100.0, 100.0 + index),
                word("here", 140.0, 100.0 + index),
...
def check_parsers(report: Report) -> None:
    ...
    report.check(
        ...
        f"paragraphs={len(joined)}, first={joined[0][-24:] if joined else ''!r}",
    )
    ...
    report.check(
        ...
        "Sub" not in speakers
        and "Moderator" in speakers
        and "Srini Pallia" in speakers,
        f"speakers={speakers}",
    )
...
def check_chunker(report: Report) -> None:
    ...
    stub_section = Section(
        section_id="business", title="Business and Segments", page_start=61, page_end=61
    )
    stub_section.blocks = [
    ...
    report.check(
        ...
        f"{len(table_chunks)} chunk(s) hold the table, kind={table_chunks[0].kind if table_chunks else '-'}",
    )
    ...
    report.check(
        ...
        bool(table_chunks)
        and table_chunks[0].text.strip().startswith("Operating Activities"),
    ...
    section = Section(
        section_id="financials", title="Financial Statements", page_start=1, page_end=1
    )
    section.blocks = [
        Block(
            kind="prose",
            text="The statement below summarises cash flows. " * 8,
            page_start=1,
            page_end=1,
        ),
        Block(
    ...
    report.check(
        ...
        not polluted,
        f"offending={polluted}"
        if polluted
        else f"{len(tail_chunks)} chunks, table left intact",
    )
    ...
    report.check(
        ...
        f"{len(packed.chunks)} chunks at 1400 chars vs {len(split.chunks)} at 500",
    )
    report.check(
...
def check_embedding_batching(report: Report) -> None:
    ...
    report.check(
        ...
        f"batch_size={client.batch_size} max_workers={client.max_workers}",
    )
    ...
    report.check(
        ...
        and all(
            abs(float(matrix[index][0]) * (14.0**0.5) - index) < 1e-3 or index == 0
    ...
    report.check(
        ...
        f"first mismatch at {
            next(
                (index for index, value in enumerate(recovered) if value != index),
                'none',
            )
        }",
    )

    def failing_request(inputs):  # noqa: ARG001 - stub matches the real _request signature
...
def check_annual_skip_default(report: Report) -> None:
    ...
    report.check(
        ...
        set(DEFAULT_ANNUAL_SKIP_SECTIONS)
        == {"financials", "notice", "governance", "esg"},
        f"default={DEFAULT_ANNUAL_SKIP_SECTIONS}",
    )
    ...
    document.sections[-1].blocks = [
        Block(
            kind="table",
            text="Revenue from operations 926,163 897,604",
            page_start=4,
            page_end=4,
        ),
    ]
    ...
    skipped = chunk_document(
        document, skip_sections=DEFAULT_ANNUAL_SKIP_SECTIONS
    ).section_counts()
    report.check(
        "chunker.skip_sections_drops_only_named_sections",
        "financials" in kept
        and "financials" not in skipped
        and skipped.get("mdna") == kept.get("mdna"),
        f"with={sorted(kept)} without={sorted(skipped)}",
    )
    ...
    report.check(
        ...
        f"narrative sections={sorted(narrative)}",
    )
...
def check_dossier(report: Report) -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for doc_id, doc_type, label, section_id, section_title, count, focus in plan:
            ...
            chunks = [
                Chunk(
                    ...
                    text=f"Passage {index} of {doc_id} about margins.",
                    page_start=index + 1,
                    page_end=index + 1,
            ...
            registry.complete(
                record, "parse", "fp", "annual/v1", detail={"pages": count}
            )
            registry.complete(
                record,
                "chunk",
        ...
        report.check(
            ...
            f"{len(financial.passages)} passages in financial_results",
        )
        ...
        report.check(
            ...
            len(docs) == 2,
            f"documents contributing={sorted(docs)}",
        )
        per_doc = {}
        ...
        report.check(
            ...
            f"per document={per_doc}",
        )

        report.check(
            "dossier.passages_are_citable",
            all(
                passage.citation and passage.chunk_id and passage.pages
                for passage in financial.passages
            ),
        ...
        report.check(
            ...
            f"{len(speaker_passages)} passages carry a speaker and role",
        )
        ...
        report.check(
            ...
            any(
                "accounting" in gap.lower() or "auditor" in gap.lower() for gap in gaps
            ),
            f"gaps={gaps[:2]}",
        )

        empty = [
            key for key, section in dossier.sections.items() if not section.passages
        ]
        report.check(
            "dossier.empty_sections_reported",
            dossier.coverage()["sections_empty"] == empty,
            f"{len(empty)} empty sections named",
        )
...
def check_rejection_handling(report: Report) -> None:
    ...
    report.check(
        ...
        408 not in _PERMANENT_STATUSES
        and 429 not in _PERMANENT_STATUSES
        and 400 in _PERMANENT_STATUSES
        and 403 in _PERMANENT_STATUSES,
    ...
    report.check(
        ...
        f"excluded={excluded} rows={matrix.shape[0]} of {len(texts)}",
    )
    report.check(
    ...
    report.check(
        ...
        calls["n"] <= 3 + 2 * 5,
        f"{calls['n']} requests to isolate one input among 40",
    )
    ...
    report.check(
        ...
        f"first mismatch at {
            next(
                (
                    i
                    for i, (a, b) in enumerate(zip(recovered, expected, strict=False))
                    if a != b
                ),
                'none',
            )
    ...
    def flaky_request(inputs):  # noqa: ARG001 - stub matches the real _request signature
    ...
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        kept_ids = [f"d::s::{index:03d}" for index in expected]
        detail = save_vectors(
            root,
            "d",
            "qwen3-embed",
        ...
        report.check(
            ...
            f"detail={detail['excluded_chunk_ids']}",
        )
        ...
        report.check(
            ...
            f"{reloaded.count if reloaded else -1} rows, {len(reloaded.excluded_chunk_ids) if reloaded else -1} excluded",
        )
    ...
    for key, _title, phrases in ANNUAL_SECTIONS:
        for phrase in phrases:
            if phrase in owners and owners[phrase] != key:
                clashes.append(f"{phrase!r}: {owners[phrase]} vs {key}")
            owners[phrase] = key
    report.check(
        ...
        "; ".join(clashes[:3])
        if clashes
        else f"{len(owners)} phrases, each owned by one section",
    )
    ...
    for phrase, key in owners.items():
        for other, other_key in owners.items():
            if phrase != other and other_key != key and other in phrase:
                contained.append(f"{phrase!r} ({key}) contains {other!r} ({other_key})")
    report.check(
...
def check_issuer_agnostic(report: Report) -> None:
    ...
    for module in modules:
        ...
        for issuer in issuers:
            if issuer in lowered:
                offenders.append(f"{module.name}: {issuer}")
    report.check(
    ...
    for company, expected in (
        ...
        if not report.check(
            f"generic.company_headings_derived[{company.split()[0]}]",
            expected in phrases,
            f"{company} -> {phrases[:2]}",
        ):
            break
    ...
    report.check(
        ...
        f"{len(signatures)} sign-off forms rejected",
    )
    report.check(
        "generic.people_are_still_speakers",
        all(
            _is_speaker_name(name)
            for name in (
                "Aparna Iyer",
                "Nitin Padmanabhan",
                "Moderator",
    ...
    wrong = [
        url
        for url, referer in hosts.items()
        if _request_headers(url).get("Referer") != referer
    ]
    report.check(
        "generic.referer_follows_the_host",
        not wrong,
        f"{len(hosts)} hosts get a same-origin referer"
        if not wrong
        else f"wrong for {wrong[:2]}",
    )
    ...
    report.check(
        ...
        f"{len(rows)} rupee notations recognised",
    )
    ...
    report.check(
        ...
        safe_ticker("M&M") == "M_M"
        and safe_ticker("BAJAJ-AUTO") == "BAJAJ-AUTO"
        and safe_ticker("l&tfh") == "L_TFH"
        and safe_ticker("") == "UNKNOWN",
        f"M&M -> {safe_ticker('M&M')}, BAJAJ-AUTO -> {safe_ticker('BAJAJ-AUTO')}",
    )


def check_prompts(report: Report) -> None:
    """Checks that prompts are built from the document actually in hand."""
    probes_all = build_probes(
        DOC_TYPE_ANNUAL_REPORT,
        "Example Industries Ltd",
        "FY2026",
        available_sections=["mdna", "risk", "business", "financials", "governance"],
    )
    probes_thin = build_probes(
        DOC_TYPE_ANNUAL_REPORT,
        "Example Industries Ltd",
        "FY2026",
    ...
    report.check(
        ...
        f"{len(probes_thin)} probes with mdna only vs {len(probes_all)} with all sections",
    )
    ...
    report.check(
        ...
        f"focus ids probed={sorted(focus_ids)}",
    )
    ...
    report.check(
        ...
        "Example Industries" not in query and "FY2026" not in query and len(query) > 40,
        repr(query[:70]),
    )
    report.check(
    ...
    chunk = Chunk(
        chunk_id="annual_report_FY2026::mdna::001",
        ticker="TEST",
        doc_id="annual_report_FY2026",
        doc_type=DOC_TYPE_ANNUAL_REPORT,
        label="FY2026",
        ...
        text="Revenue grew to 926,163 million.",
        page_start=41,
        page_end=41,
    )
    ...
    for doc_type, profile in DOC_PROFILES.items():
        if not report.check(
            f"prompts.profile_states_limits[{doc_type}]",
            bool(profile.authority)
            and bool(profile.limits)
            and bool(profile.focus_areas),
            f"{len(profile.focus_areas)} focus areas",
        ):
            break
...
def check_registry(report: Report) -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        record = registry.upsert(
            ...
            doc_type=DOC_TYPE_ANNUAL_REPORT,
            label="FY2026",
            source_url="https://example.test/ar-2026.pdf",
        )
        report.check(
        ...
        report.check(
            ...
            not registry.needs(
                record, "download", record.url_fingerprint, "fetcher/v1"
            ),
        ...
        changed = again.upsert(
            ...
            doc_type=DOC_TYPE_ANNUAL_REPORT,
            label="FY2026",
            source_url="https://example.test/ar-2026-revised.pdf",
        )
        report.check(
            "registry.url_change_resets_stages",
            not changed.stage("download").is_done
            and not changed.stage("embed").is_done,
            "a new link re-ingests the document",
        )
        report.check(
            "registry.url_change_recorded_in_history",
            any(
                event.get("event") == "source_url_changed" for event in changed.history
            ),
            "the change is auditable",
        )

        missing = again.upsert(
            doc_id="transcript_2026_07",
            doc_type=DOC_TYPE_TRANSCRIPT,
            label="Jul 2026",
            source_url="https://example.test/t.pdf",
        )
        again.complete(
            missing,
            "chunk",
...
def run_offline_checks() -> Report:
    ...
    for name, function in (
        ...
        try:
            ...
            report.check(name + ".crashed", False, f"{type(exc).__name__}: {exc}")
    return report
...
def check_idempotent_run(
    ...
    report.check(
        ...
        f"downloaded={actions.get('download', {}).get('downloaded', 0)}",
    )
    report.check(
        "live.first_run_embeds",
        actions.get("embed", {}).get("embedded", 0) > 0,
        f"embedded={actions.get('embed', {}).get('embedded', 0)} documents, {actions.get('embed', {}).get('vectors', 0)} vectors",
    )
    report.check(
    ...
    report.check(
        ...
        not plan,
        f"planned={json.dumps(plan)[:160]}" if plan else "empty plan",
    )
    for stage in ("download", "parse", "chunk", "embed"):
        did = second_actions.get(stage, {})
        worked = sum(
            count for key, count in did.items() if key not in ("current", "skipped")
        )
        report.check(
            f"live.second_run_no_{stage}",
            worked == 0,
            f"{stage}={json.dumps(did)}",
        )
    dossier = second_actions.get("dossier", {})
    report.check(
        "live.second_run_dossier_unchanged",
        dossier.get("written", 0) == 0 or dossier.get("unchanged", 0) > 0,
        f"dossier={json.dumps(dossier)}",
    )
    ...
    report.check(
        ...
        f"chunk={json.dumps(forced_actions.get('chunk', {}))} embed={json.dumps(forced_actions.get('embed', {}))}",
    )
    ...
    report.check(
        ...
        f"documents={counts['documents']} embedded={counts['embed']}",
    )
    report.check(
        "live.every_document_has_a_prompt",
        counts["prompt"] == counts["documents"],
        f"prompts={counts['prompt']} of {counts['documents']}",
    )
    ...
    report.check(
        ...
        not unnamed,
        f"unnamed={unnamed}" if unnamed else "every vector set names its model",
    )
...
def check_vector_alignment(report: Report, ticker: str, data_dir: Path) -> None:
    ...
    root = registry.root
    embedded = [record for record in registry.select() if record.stage("embed").is_done]
    if not embedded:
    ...
    for record in embedded:
        ...
        if store is None or chunk_set is None:
            report.check(f"vectors.loadable[{record.doc_id}]", False, "missing files")
            continue
        ...
        if not report.check(
            f"vectors.rows_match_chunks[{record.doc_id}]",
            store.chunk_ids == expected,
            f"{store.count} vectors, {len(chunk_set.chunks)} chunks, {len(refused)} refused",
        ):
            continue
        report.check(
            f"vectors.fingerprint_matches[{record.doc_id}]",
    ...
    document = load_document_index(
        root / "chunks", root / "vectors", embedded[0].doc_id
    )
    report.check(
    ...
    probes = build_probes(
        document.doc_type,
        registry.company_name or ticker,
        document.label,
    ...
    empty = document.search(
        document.vectors[0],
        top_k=5,
        sections=["section_that_does_not_exist"],
    )
    report.check(
        "retrieval.absent_section_returns_nothing",
        empty == [],
        f"{len(empty)} hits for an absent section",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.offline:
        print(f"\nLive checks ({args.ticker}, {', '.join(args.doc_types)})")
        live = Report()
        ...
        try:
            ...
            live.check("live.crashed", False, f"{type(exc).__name__}: {exc}")
        finally:
            if temporary:
