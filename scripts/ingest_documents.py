...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--data-dir", help=f"Root artifact directory (default: {OUTPUT_DIR})"
    )
    p.add_argument(
        "--annual-reports",
        type=int,
    ...
    p.add_argument(
        "--no-prompts", action="store_true", help="Skip building extraction prompts"
    )
    p.add_argument(
        "--max-pages", type=int, help="Cap pages parsed per document (smoke tests)"
    )
    p.add_argument(
        "--skip-sections",
        nargs="*",
        help="Annual-report sections to leave out of chunking. Defaults "
        f"to {' '.join(DEFAULT_ANNUAL_SKIP_SECTIONS)}. Pass the flag with no values to embed every "
        "section, including the audited statements.",
    )
    p.add_argument(
        "--status", action="store_true", help="Print the registry's state and exit"
    )
...
def _grid(header: Sequence[str], rows: Sequence[Sequence[str]]) -> str:
    ...
    return "\n".join(
        [line(header), "  ".join("-" * w for w in widths)] + [line(row) for row in rows]
    )
...
def _print_status(registry: DocumentRegistry) -> None:
    ...
    if not rows:
        print(f"No documents catalogued for {registry.ticker} yet.")
        return
    ...
    print(
        f"\n{registry.ticker}: {counts['documents']} documents; "
        f"{', '.join(f'{stage}={counts[stage]}' for stage in STAGES)}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    cli.setup(
        logging.DEBUG if args.verbose else logging.INFO, cli.TIMED, datefmt=cli.CLOCK
    )
    ...
    if args.json:
        ...
        if errors:
            ...
            for item in errors:
                print(f"  {item['doc_id']:<28} {item['stage']:<9} {item['error']}")
