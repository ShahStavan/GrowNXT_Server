...
def resolve_symbol(ticker_or_name: str) -> str:
    ...
    try:
        ...
        if (OUTPUT_DIR / upper / "api" / "summary.json").exists() or (
            OUTPUT_DIR / upper / "summary.json"
        ).exists():
            return upper
    except Exception:
        pass
    ...
    try:
        ...
        if hits:
            ...
            if resolved != upper:
                logging.getLogger("generate_report").info(
                    "Resolved company '%s' -> ticker '%s'", raw, resolved
                )
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "tickers",
        nargs="+",
        help="Ticker symbols or company names, e.g. WIPRO, Infosys",
    )
    p.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> int:
    ...
    cli.setup(
        logging.WARNING if args.quiet else logging.INFO, cli.PLAIN, stream=sys.stderr
    )
    ...
    for ticker_input in args.tickers:
        ...
        try:
            ...
            print(f"{sym:<12} {label}: {exc}", file=sys.stderr)
            failed.append(sym)
        else:
            print(f"{sym:<12} {path}")
