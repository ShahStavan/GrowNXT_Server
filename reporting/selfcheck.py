...
class SelfCheck:
    ...
    @property
    def worst_identity_residual(self) -> Optional[float]:
        """Largest absolute residual among the identity checks."""
        deltas = [abs(c.delta) for c in self.checks
...
def _add(checks: List[Check], **kwargs) -> None:
    ...
    if check.passed is False:
        logger.warning(
            ...
            check.name, check.expected, check.actual, check.delta, check.unit,
        )
...
def _cashflow_articulation(snap: CompanySnapshot, checks: List[Check]) -> None:
    ...
    for row in snap.cashflow:
        if None in (row.cfo, row.cash_from_investing,
                    row.cash_from_financing, row.net_change_in_cash):
            continue
        tested += 1
        residual = ((row.cfo + row.cash_from_investing + row.cash_from_financing)
                    - row.net_change_in_cash)
        if worst is None or abs(residual) > abs(worst):
            worst, worst_period = residual, row.period
    _add(
        checks, name="Cash flow articulates to the reported movement in cash",
        kind="identity", expected=0.0, actual=worst,
        tolerance=TOL_IDENTITY_CRORE, unit="Rs cr",
        detail=("Operating plus investing plus financing, against the "
                "provider's own closing figure, across %d periods. Worst is %s."
                % (tested, fmt.period_label(worst_period) if worst_period else "none")),
    )
...
def _free_cash_flow(snap: CompanySnapshot, checks: List[Check]) -> None:
    ...
    for row in snap.cashflow:
        ...
        if worst is None or abs(residual) > abs(worst):
            worst, worst_period = residual, row.period
    _add(
        checks, name="Free cash flow equals operating cash flow less capex",
        kind="identity", expected=0.0, actual=worst,
        tolerance=TOL_IDENTITY_CRORE, unit="Rs cr",
        detail=("Checked across %d periods. Worst is %s."
                % (tested, fmt.period_label(worst_period) if worst_period else "none")),
    )
...
def _sources_and_uses(composites: Composites, checks: List[Check]) -> None:
    ...
    _add(
        checks, name="Capital allocation: sources equal uses",
        kind="identity", expected=0.0, actual=allocation.residual,
        tolerance=TOL_IDENTITY_CRORE, unit="Rs cr",
        detail=("Cumulative over %d years to %s. The two columns are built "
                "from one articulation, so a residual would mean a "
                "classification error."
                % (allocation.years,
                   fmt.period_label(allocation.to_period)
                   if allocation.to_period else "none")),
    )
...
def _dupont(composites: Composites, checks: List[Check]) -> None:
    ...
    for row in dupont.years:
        ...
        if worst is None or abs(residual) > abs(worst):
            worst, worst_period = residual, row.period
    _add(
        checks, name="DuPont: five factors multiply to return on equity",
        kind="identity", expected=0.0, actual=worst,
        tolerance=TOL_IDENTITY_PCT, unit="pp",
        detail=("Every period rebuilds ROE from its own factors and is set "
                "against ROE computed directly from profit and equity. "
                "Checked across %d periods. Worst is %s."
                % (tested, fmt.period_label(worst_period) if worst_period else "none")),
    )

    if dupont.provider_roe is not None and dupont.reconciliation_delta is not None:
        _add(
            checks, name="DuPont: rebuilt ROE matches the provider's published ROE",
            kind="reconciliation",
            expected=0.0, actual=dupont.reconciliation_delta,
            tolerance=TOL_PROVIDER_PCT, unit="pp",
            detail=("Computed here for %s against the provider's own "
                    "five-factor endpoint, which publishes %s. The provider "
                    "rounds its factors, so a residual of a few thousandths "
                    "of a point is expected."
                    % (dupont.provider_period or "the trailing period",
                       fmt.pct(dupont.provider_roe, 2))),
        )
    ...
    match = next((r for r in dupont.years
                  if r.period.strip().upper()
                  == (dupont.provider_period or "").strip().upper()), None)
    if match is not None and dupont.provider_factors:
        ...
        for key, label, mine in pairs:
            ...
            _add(
                checks, name="DuPont factor reconciles: %s" % label.lower(),
                kind="reconciliation", expected=theirs, actual=mine,
                tolerance=_rounding_tolerance(decimals),
                unit="pp" if key == "operating_margin" else "x",
                detail=("Computed here at full precision against the "
                        "provider's published factor, which it reports to %d "
                        "decimal places." % decimals),
            )


def _returns(snap: CompanySnapshot, derived: DerivedAnalytics,
             checks: List[Check]) -> None:
    """Reconciles the trailing return series against the ratio endpoints."""
    trailing = next((m for m in reversed(derived.annual)
                     if m.period.strip().upper() == "TTM"), None)
    if trailing is None:
    ...
    if provider_roe is not None and trailing.roe is not None:
        _add(
            checks, name="Return on equity matches the provider's endpoint",
            kind="reconciliation", expected=provider_roe, actual=trailing.roe,
            tolerance=TOL_PROVIDER_PCT, unit="pp",
            detail=("Both on period-end equity, which is the convention the "
                    "provider's endpoint uses."),
        )

    provider_roic = snap.capital_efficiency.get(
        "return_on_invested_capital_roic_pct")
    if provider_roic is not None and trailing.roic is not None:
        _add(
            checks, name="Return on invested capital matches the provider's endpoint",
            kind="reconciliation", expected=provider_roic, actual=trailing.roic,
            tolerance=TOL_METHODOLOGY_PCT, unit="pp",
            detail=("Invested capital is equity plus debt less cash, and NOPAT "
...
def _piotroski(composites: Composites, checks: List[Check]) -> None:
    ...
    _add(
        checks, name="F-Score equals the sum of its sub-tests",
        kind="identity",
        expected=float(sum(awarded)) if awarded else None,
        actual=float(score.score) if score.score is not None else None,
        tolerance=0.0, unit="points",
        detail=("%d of the nine signals were evaluated and %d awarded a point. "
                "The total is a property of the sub-test list, so this check "
                "confirms none were dropped between computation and rendering."
                % (score.computable, sum(awarded) if awarded else 0)),
    )

    stray = [t.number for t in score.tests
             if t.points is not None and t.points not in (0, 1)]
    _add(
        checks, name="Every F-Score signal awards nil or one point",
        kind="guardrail", expected=0.0, actual=float(len(stray)),
        tolerance=0.0, unit="signals",
        detail=("A signal outside nil-or-one would mean a scoring bug. "
                "Offending signals: %s."
    ...
    _add(
        checks, name="Every F-Score signal carries its definition and inputs",
        kind="guardrail", expected=0.0, actual=float(len(missing_definition)),
        tolerance=0.0, unit="signals",
        detail=("The report may not print a composite without the components "
...
def _altman(composites: Composites, checks: List[Check]) -> None:
    ...
    if any(c is None for c in contributions):
        return
    _add(
        checks, name="Z-Score equals the sum of its weighted terms",
        kind="identity", expected=sum(contributions), actual=altman.score,
        tolerance=TOL_IDENTITY_PCT, unit="points",
        detail=("Five terms, each a ratio times its published coefficient. "
    ...
    _add(
        checks, name="Every Z-Score term carries its ratio and coefficient",
        kind="guardrail", expected=0.0, actual=float(len(missing)),
        tolerance=0.0, unit="terms",
        detail=("Terms lacking a stated definition: %s."
...
def _reinvestment(composites: Composites, checks: List[Check]) -> None:
    ...
    if rate is None or roic is None or reinvestment.implied_growth is None:
        return
    _add(
        checks, name="Implied growth equals reinvestment rate times return",
        kind="identity", expected=rate / 100.0 * roic,
        actual=reinvestment.implied_growth,
        tolerance=TOL_IDENTITY_PCT, unit="pp",
        detail=("Reinvestment rate of %s applied to a return on invested "
                "capital of %s."
                % (fmt.pct(rate, 1), fmt.pct(roic, 1))),
    )

    if reinvestment.total_nopat is not None and reinvestment.total_reinvestment is not None:
        summed = sum(y.reinvestment for y in reinvestment.years
                     if y.reinvestment is not None)
        _add(
            checks, name="Cumulative reinvestment totals the annual figures",
            kind="identity", expected=summed,
...
def _denominator_guardrail(snap: CompanySnapshot, derived: DerivedAnalytics,
                           composites: Composites, checks: List[Check]) -> None:
    ...
    for metrics in derived.annual:
        ...
        if equity is not None and equity <= 0:
            for label, value in (("return on equity", metrics.roe),
                                 ("debt to equity", metrics.debt_to_equity),
                                 ("goodwill to equity", metrics.goodwill_to_equity)):
                if value is not None:
                    breaches.append("%s in %s on equity of %s"
                                    % (label, metrics.period, fmt.num(equity)))
        if metrics.invested_capital is not None and metrics.invested_capital <= 0 \
                and metrics.roic is not None:
            breaches.append("return on invested capital in %s on invested "
                            "capital of %s"
                            % (metrics.period, fmt.num(metrics.invested_capital)))
    ...
    for row in composites.dupont.years:
        ...
        if balance.equity <= 0 and row.equity_multiplier is not None:
            breaches.append("DuPont equity multiplier in %s on equity of %s"
                            % (row.period, fmt.num(balance.equity)))

    _add(
        checks, name="No ratio published against a non-positive denominator",
        kind="guardrail", expected=0.0, actual=float(len(breaches)),
        tolerance=0.0, unit="figures",
        detail=("Negative equity, negative EBITDA and negative cost of goods "
...
def run(
    ...
    if result.failures:
        logger.warning("[%s] %d self-check failure(s)",
                       snap.ticker, len(result.failures))
    else:
        logger.info("[%s] %d self-checks passed, %d skipped",
                    snap.ticker, result.passed_count, len(result.skipped))
    return result
